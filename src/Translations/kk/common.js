export default {
  /* Chapters numbered */
  chapter1: "1-бөлім",
  chapter2: "2-бөлім",
  chapter3: "3-бөлім",
  chapter4: "4-бөлім",
  chapter5: "5-бөлім",
  chapter6: "6-бөлім",
  chapter7: "7-бөлім",
  chapter8: "8-бөлім",
  chapter9: "9-бөлім",
  chapter10: "10-бөлім",
  chapter11: "11-бөлім",
  chapter12: "12-бөлім",
  chapter13: "13-бөлім",
  chapter14: "14-бөлім",
  chapter15: "15-бөлім",

  /* Minigame names */
  memory: "Білімді тексеру",
  sorting: "Топтастыру",
  sorting2: "Топтастыру 2",
  quiz: "Викторина",
  imageboom: "Ашық жауап",
  imageboom2: "Ашық жауап 2",
  snapshot: "Сурет түсіру",
  snapshot2: "Сурет түсіру 2",
  reorder: "Реттеу",
  mastery: "Шеберлік",
  mastery2: "Шеберлік 2",
  /* drawing: "Сурет салу",
  flashcards: "Карточки", */

  PLACEHOLDER: "Missing Translation",

  hello: "Сәлем",

  reminders: "Еске салғыштар",
  soundeffects: "Дыбыс әсерлері",
  languageSelector: "Тіл таңдау",

  general: "Жалпы",
  help: "Көмек",
  about: "Біз туралы",

  start: "Бастау",
  back: "Артқа бару",
  submit: "Жіберу",
  enteryouranswer: "Жауабыңызды енгізіңіз...",
  pickanimage: "Суретті таңдаңыз",
  restart: "Қайтадан бастаңыз",
  next:"Келесі",
  tryagain:"Қайтадан көріңіз",

  login: "Кіру",
  signup: "Тіркелу",
  makeanewaccount: "Жаңа есептік жазба жасаңыз",
  teachersignup: "Мұғалім тіркелу",
  signout: "Шығу",
  continueasguest: "Қонақ ретінде жалғастыру",
  forgotpassword: "Құпия сөзіңізді ұмыттыңыз ба?",
  googlesignin: "Google арқылы кіру",
  resetemail: "Құпия сөзді қалпына келтіру электрондық поштасы жіберілді",
  senttoinbox: "Құпия сөзіңізді қалпына келтіру нұсқаулары бар электрондық пошта жәшігіңізге жіберілді.",
  welcome: "Қош келдіңіз",
  accountsuccessfullycreated: "Есептік жазбаңыз сәтті жасалды.",
  other: "Басқа",

  havegoogle: "Google есептік жазбаңыз бар ма?",
  backtogooglelogin: "Тіркелудің қажеті жоқ! Google кіру бетіне қайта оралыңыз.",

  email: "Электрондық пошта",
  password: "Құпия сөз",
  username: "Пайдаланушы аты",
  firstname: "Аты",
  lastname: "Тегі",
  classcode: "Сынып коды",
  teacher: "Мұғалім",

  assignments: "Тапсырмалар",
  completions: "Орындалулар",
  levels: "Деңгейлер",
  grade: "Сынып",
  chapter: "Тарау",
  lessons: "Сабақтар",
  lesson: "Сабақ",

  home: "Басты бет",
  profile: "Профиль",
  settings: "Параметрлер",
  leaderboard: "Көшбасшылар тақтасы",
};